...
- Creates Azure VM Windows --> module "vmwindows"
- Set the default diagnostics settings (All Logs and metric) whith a Log Analytics workspace as destination
- Assigns subnet and network security group to created VM
- Set VM public IP --> enable_public_ip option
...
locals {
  ...
  domain_name_label                       = "pip-vmwin003"
...
module "pip" {
  source                          = "git::ssh://git@ssh.dev.azure.com/v3/ECTL-AZURE/ECTL-Terraform-Modules/terraform-azurerm-publicip//module?ref=master"
  landing_zone_slug               = local.landing_zone_slug
  stack                           = local.stack
  location                        = module.regions.location
  location_short                  = module.regions.location_short
...
| <a name="module_pip"></a> [pip](#module\_pip) | git::ssh://git@ssh.dev.azure.com/v3/ECTL-AZURE/ECTL-Terraform-Modules/terraform-azurerm-publicip//module | master |
